...
def mock_data_intan_triggered():
    ...
    with patch.object(
        intan_module.DataIntanTriggered, "__init__", lambda x, **kwargs: None
    ):
            ...
            with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                ...
                with patch("miv.io.intan.data.ET.parse") as mock_parse:
                    ...
                    with patch("miv.utils.mpi.task_index_split") as mock_split:
                        ...
                        # Call the method
                        result = list(
                            mock_data_intan._generator_by_channel_name(
                                "amplifier_data", mpi_comm=mock_mpi_comm
                            )
                        )
    ...
    with patch.object(mock_data_intan, "_generator_by_channel_name") as mock_generator:
        ...
        with patch.object(mock_data_intan, "_get_active_channels") as mock_channels:
            ...
            with patch.object(mock_data_intan, "_expand_channels") as mock_expand:
                ...
                mock_generator.assert_called_once_with(
                    "amplifier_data", progress_bar=False, mpi_comm=None
                )
        ...
        with patch.object(mock_data_intan, "_get_active_channels") as mock_channels:
            ...
            with patch.object(mock_data_intan, "_expand_channels") as mock_expand:
                ...
                if len(call_args[0]) > 2:
                    assert (
                        call_args[0][2] == mock_mpi_comm
                    )  # mpi_comm as positional arg
                elif "mpi_comm" in call_args[1]:
                    assert (
                        call_args[1]["mpi_comm"] == mock_mpi_comm
                    )  # mpi_comm as keyword arg
                else:
    ...
    with patch.object(mock_data_intan, "_read_header") as mock_header:
        ...
        with patch.object(mock_data_intan, "_load_digital_event_common") as mock_common:
            ...
            mock_common.assert_called_once_with(
                "board_dig_in_data", 2, progress_bar=False, mpi_comm=mock_mpi_comm
            )
        ...
        with patch.object(mock_data_intan, "check_path_validity", return_value=True):
            ...
            with patch("miv.io.intan.data.ET.parse") as mock_parse:
                ...
                with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                    ...
                    result = list(
                        mock_data_intan._generator_by_channel_name("amplifier_data")
                    )
                    assert len(result) == 1
                    assert result[0].rate == 30000.0
                ...
                with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                    ...
                    with pytest.raises(
                        AssertionError, match="No.*amplifier_data.*in the file"
                    ) as exc_info:
                        list(
                            mock_data_intan._generator_by_channel_name("amplifier_data")
                        )
            ...
            with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                ...
                with patch("miv.io.intan.data.ET.parse") as mock_parse:
                    ...
                    with patch("miv.utils.mpi.task_index_split") as mock_split:
                        ...
                        # Mock _get_active_channels
                        with patch.object(
                            mock_data_intan, "_get_active_channels"
                        ) as mock_channels:
                            mock_channels.return_value = (np.array([0, 1]), 2)

                            # Mock _expand_channels
                            with patch.object(
                                mock_data_intan, "_expand_channels"
                            ) as mock_expand:
                                # Call load() with MPI comm
                                result = list(
                                    mock_data_intan.load(mpi_comm=mock_comm_rank0)
                                )
                                ...
                                loaded_files = [
                                    call[0][0] for call in mock_load_file.call_args_list
                                ]
...
def test_load_mpi_all_files_processed(mock_data_intan):
    ...
    for rank in range(num_ranks):
        ...
        with patch.object(mock_data_intan, "get_recording_files") as mock_files:
            ...
            # Mock check_path_validity
            with patch.object(
                mock_data_intan, "check_path_validity", return_value=True
            ):
                # Mock the rhs.load_file function
                with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                    mock_load_file.return_value = (
                    ...
                    with patch("miv.io.intan.data.ET.parse") as mock_parse:
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            ...
                            expected_indices = np.array_split(
                                np.arange(len(all_files)), num_ranks
                            )[rank].tolist()
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    ...
                                    loaded_files = [
                                        call[0][0]
                                        for call in mock_load_file.call_args_list
                                    ]
...
def test_data_intan_triggered_mpi_segment_splitting(mock_data_intan_triggered):
    ...
    with patch.object(mock_data_intan_triggered, "_trigger_grouping") as mock_grouping:
        ...
        # Mock check_path_validity
        with patch.object(
            mock_data_intan_triggered, "check_path_validity", return_value=True
        ):
            # Mock the rhs.load_file function
            with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                mock_load_file.return_value = (
                ...
                with patch("miv.io.intan.data.ET.parse") as mock_parse:
                    ...
                    with patch("miv.utils.mpi.task_index_split") as mock_split:
                        ...
                        # Mock _get_active_channels
                        with patch.object(
                            mock_data_intan_triggered, "_get_active_channels"
                        ) as mock_channels:
                            mock_channels.return_value = (np.array([0, 1]), 2)

                            # Call the method
                            result = list(
                                mock_data_intan_triggered._generator_by_channel_name(
...
def test_data_intan_triggered_mpi_single_segment(mock_data_intan_triggered):
    ...
    mock_groups = [{"paths": ["file1.rhs"], "start index": [0], "end index": [100]}]
    ...
    for rank in range(num_ranks):
        ...
        with patch.object(
            mock_data_intan_triggered, "_trigger_grouping"
        ) as mock_grouping:
            mock_grouping.return_value = mock_groups
            ...
            with patch.object(
                ...
                with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                    ...
                    with patch("miv.io.intan.data.ET.parse") as mock_parse:
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            # Calculate expected indices for this rank using np.array_split logic
                            expected_indices = np.array_split(np.arange(1), num_ranks)[
                                rank
                            ].tolist()
                            mock_split.return_value = expected_indices
...
def test_data_intan_triggered_mpi_many_segments(mock_data_intan_triggered):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch("miv.io.intan.data.rhs.load_file") as mock_load_file:
                    ...
                    mock_load_file.return_value = (
                        {
                            "amplifier_data": np.array([[1, 2] * 2000]).reshape(
                                2, 2000
                            ),
                            "t": np.arange(2000),
                        },
                    ...
                    with patch("miv.io.intan.data.ET.parse") as mock_parse:
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            # Calculate expected indices for this rank using np.array_split logic
                            expected_indices = np.array_split(
                                np.arange(num_segments), num_ranks
                            )[rank].tolist()
                            mock_split.return_value = expected_indices
                            ...
                            with patch.object(
                                ...
                                assert mock_load_file.call_count == len(
                                    expected_indices
                                ), (
    ...
    for rank1 in range(num_ranks):
        for rank2 in range(rank1 + 1, num_ranks):
            overlap = set(segments_processed_by_rank[rank1]) & set(
                segments_processed_by_rank[rank2]
            )
            assert len(overlap) == 0, (
...
def test_error_handling_mpi_communicator_none_data_intan_triggered(
    mock_data_intan_triggered,
):
...
def test_error_handling_more_ranks_than_files_data_intan(mock_data_intan):
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    ...
                                    try:
                                        result = list(
                                            mock_data_intan.load(mpi_comm=mock_comm)
                                        )
                                    except Exception as e:
                                        pytest.fail(
                                            f"Rank {rank} raised an exception when it should handle empty file list gracefully: {e}"
                                        )
    ...
    for rank in ranks_with_files:
        ...
        expected_indices = np.array_split(np.arange(num_files), num_ranks)[
            rank
        ].tolist()
...
@pytest.mark.mpi
def test_error_handling_more_ranks_than_files_data_intan_triggered(
    mock_data_intan_triggered,
):
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            ...
                            with patch.object(
                                ...
                                try:
                                    ...
                                    pytest.fail(
                                        f"Rank {rank} raised an exception when it should handle empty segment list gracefully: {e}"
                                    )
    ...
    assert len(ranks_without_segments) > 0, (
        "At least one rank should get empty segment list"
    )
    ...
    for rank in ranks_with_segments:
        # Recalculate what indices this rank should have gotten
        expected_indices = np.array_split(np.arange(num_segments), num_ranks)[
            rank
        ].tolist()
        all_processed_indices.extend(expected_indices)
...
@pytest.mark.mpi
def test_error_handling_file_loading_failure_data_intan_triggered(
    mock_data_intan_triggered,
):
                    ...
                    with patch("miv.io.intan.data.ET.parse") as mock_parse:
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            ...
                            def load_file_side_effect(filename):
                                if (
                                    filename
                                    == mock_groups[0]["paths"][failing_segment_index]
                                ):
                                    # Simulate file loading failure: return data_present=False
                                    return ({}, False)
                                else:
                            ...
                            with patch.object(
                                ...
                                if failing_segment_index in expected_indices:
                                    # This rank should raise an AssertionError when it tries to load the failing file
                                    with pytest.raises(
                                        AssertionError, match="Data does not present"
                                    ):
                                        list(
                                            mock_data_intan_triggered._generator_by_channel_name(
                                                "amplifier_data", mpi_comm=mock_comm
...
def test_complete_mpi_data_flow_file_processing(mock_data_intan):
    ...
    for i, filename in enumerate(all_files):
        file_data[filename] = {
            "amplifier_data": np.array(
                [[i * 10 + j for j in range(2)] for _ in range(10)]
            ),
            "t": np.arange(10) + i * 10,
        }
                        ...
                        with patch("miv.utils.mpi.task_index_split") as mock_split:
                            ...
                            with patch.object(
                                ...
                                with patch.object(
                                    ...
                                    files_processed_by_rank[rank] = [
                                        all_files[i] for i in expected_indices
                                    ]
...
def test_complete_mpi_data_flow_data_combination(mock_data_intan):
    ...
    for i, (non_mpi_signal, mpi_signal) in enumerate(
        zip(non_mpi_signals, mpi_signals_combined)
    ):
        ...
        (
            np.testing.assert_array_equal(
                non_mpi_signal.timestamps, mpi_signal.timestamps
            ),
...
def test_mpi_with_configure_load_parameter_storage(mock_data_intan, mock_mpi_comm):
    ...
    assert "_load_param" in dir(mock_data_intan)
    assert (
        "mpi_comm" not in mock_data_intan._load_param
        or mock_data_intan._load_param.get("mpi_comm") is None
    )
...
def test_mpi_with_configure_load_pipeline_execution(mock_data_intan, mock_mpi_comm):
    ...
    with (
        patch.object(mock_data_intan, "load") as mock_load,

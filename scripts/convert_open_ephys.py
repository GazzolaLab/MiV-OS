...
def main(folder_path: str, verbose: bool):
    ...
    for recording in data_manager:
        ...
        with recording.load() as (signal, timestamps, sampling_rate):
            ...
            miv_file.create_dataset(miv_data, ["signal", "timestamps", "sampling_rate"], group=data_path, dtype=float)
    ...
    f = miv_file.write(f"{folder_path}/MiV_data.h5", miv_data, comp_type="gzip", comp_opts=9, logger=logger)

        
if __name__ == '__main__':
    main(args=sys.argv[(seq_find(lambda x: os.path.basename(x) == script_name, sys.argv)+1):])
